...
public class BrittleCrown extends CustomRelic implements LunarRelic {
    ...
    public void onAttack(DamageInfo info, int damageAmount, AbstractCreature target) {
        if ((AbstractDungeon.getCurrRoom()).phase == AbstractRoom.RoomPhase.COMBAT) {
            if (damageAmount > 0) {
                this.flash();
                accumulateAttackAmount += Math.min(damageAmount, target.currentHealth);
...
//            System.out.println("gain: " + accumulateAttackAmount + " " + gainStacks);
    ...
    public int onAttacked(DamageInfo info, int damageAmount) {
        if ((AbstractDungeon.getCurrRoom()).phase == AbstractRoom.RoomPhase.COMBAT) {
            if (damageAmount > 0) {
                this.flash();
                accumulateAttackedAmount += Math.min(damageAmount, AbstractDungeon.player.currentHealth);
...
//            System.out.println("lose: " + accumulateAttackedAmount + " " + lostStacks);
    ...
    public void onVictory() {
        int coins = stacks / perCoinNeedStack * perCoin;
        if (coins > 0) {
