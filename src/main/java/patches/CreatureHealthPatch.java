...
public class CreatureHealthPatch {
    ...
    @SpirePatch(cls = "com.megacrit.cardcrawl.core.AbstractCreature", method = "renderHealth", paramtypez = {SpriteBatch.class})
    public static class RenderBlueShieldBarPatch {
        public static void Replace(AbstractCreature __instance, SpriteBatch sb) {
            try {
                ...
                String[] TEXT = ReflectionHacks.getPrivateStatic(AbstractCreature.class, "TEXT");

                float x = __instance.hb.cX - __instance.hb.width / 2.0F;
                float y = __instance.hb.cY - __instance.hb.height / 2.0F + hbYOffset;

//                float originBlueWidth = healthBarWidth * __instance.maxHealth / (__instance.maxHealth + maxShield) + (curShield * 1.0f) / (__instance.maxHealth + maxShield);
...
//                float originRedWidth = healthBarWidth * __instance.maxHealth / (__instance.maxHealth + maxShield);
                        ...
                        if (curShield > 0) {//虫盾不为0
                            ...
                            if (!__instance.hasPower("Poison")) {
                                ...
                                if (__instance.currentHealth > poisonAmt) {
                                    ...
                                    if (__instance.currentHealth > 0)
                                        sb.draw(ImageMaster.HEALTH_BAR_L, x - HEALTH_BAR_HEIGHT, y + HEALTH_BAR_OFFSET_Y, HEALTH_BAR_HEIGHT, HEALTH_BAR_HEIGHT);
                                    sb.draw(ImageMaster.HEALTH_BAR_B, x, y + HEALTH_BAR_OFFSET_Y, originBlueWidth - w, HEALTH_BAR_HEIGHT);
                                    sb.draw(ImageMaster.HEALTH_BAR_R, x + originBlueWidth - w, y + HEALTH_BAR_OFFSET_Y, HEALTH_BAR_HEIGHT, HEALTH_BAR_HEIGHT);
                                }
                            }
                        } else {
                            //橙条
                        ...
                        if (curShield > 0) {//虫盾
                            ...
                            if (!__instance.hasPower("Poison")) {
                                ...
                                if (__instance.currentHealth > poisonAmt) {
                                    ...
                                    sb.draw(ImageMaster.HEALTH_BAR_B, x, y + HEALTH_BAR_OFFSET_Y, originRedWidth - w, HEALTH_BAR_HEIGHT);
                                    sb.draw(ImageMaster.HEALTH_BAR_R, x + originRedWidth - w, y + HEALTH_BAR_OFFSET_Y, HEALTH_BAR_HEIGHT, HEALTH_BAR_HEIGHT);
                                }
                            }
                        }
                    } else {//没虫盾
                ...
                if (maxShield > 0) {
                    if (blueWidth != 0.0F) {
                        ...
                        FontHelper.renderFontCentered(sb, FontHelper.healthInfoFont, TEXT[0], __instance.hb.cX, y + HEALTH_BAR_OFFSET_Y + HEALTH_TEXT_OFFSET_Y - Settings.scale, hbTextColor);
                    }
                } else {
